...
So the mean is `r newmeanperday` (a difference of `r newmeanperday - meanperday` ) and the median is `r newmedianperday` (a difference of `r newmedianperday - medianperday` ).
...
aggnew <- aggregate(newdata , by=list(Interval = newdata$interval, Daytype = newdata$weekday), FUN=mean)
g <- qplot(Interval, 
           steps, 
           data = aggnew, 
           color = Daytype, 
